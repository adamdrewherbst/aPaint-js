...
(function($)
{
	...
	{	
		...
		calcOffset: function() {
			var _self = this, $canvas = $(_self.canvas), $elem = _self.$elem;
			_self.elemOffset.top = $canvas.offset().top - $elem.offset().top;
			_self.elemOffset.left = $canvas.offset().left - $elem.offset().left;
			var menu = _self.mainMenu.menu;
			menu.css({
				left: _self.settings.menuOffsetX + _self.elemOffset.left,
				top: _self.settings.menuOffsetY + _self.elemOffset.top
			});
			
			console.log('elem offset:');
			console.info($elem);
			console.info($elem.offset());
			console.info($canvas.offset());
			console.info(_self.elemOffset);
		},
		
		/**********************
		 * for interaction with a fabric.js Canvas
		 */
		
		setFabricCanvas: function(fabCanvas) {
			var _self = this;
			_self.fabCanvas = fabCanvas;
			//_self.textInput.detach().appendTo(_self.fabCanvas.upperCanvasEl);
		},
		
		addMyListeners: function() {
			var _self = this;
			var curCanvas = _self.settings.mode !== 'Text' && _self.fabCanvas ? _self.fabCanvas.upperCanvasEl : this.canvas;
			$(curCanvas).on('mousedown.wPaint', function(e)
			{
				//console.log('wp mouse down');
				e.preventDefault();
				e.stopPropagation();
			...
			$(document)
			.on('mousemove.wPaint', function(e)
			{
				if(_self.draw) _self.callFunc(e, _self, 'Move');
			})
			.on('mouseup.wPaint', function(e)
			{
				//make sure we are in draw mode otherwise this will fire on any mouse up.
				if(_self.draw)
				{
		...
		removeMyListeners: function() {
			var _self = this;
			$(this.canvas).off('mousedown.wPaint');
			$(_self.fabCanvas.upperCanvasEl).off('mousedown.wPaint');
			$(document).off('mousemove.wPaint').off('mouseup.wPaint');
		},
		...
		callFunc: function(e, _self, event)
		{
			...
			var funcStr = 'draw' + mode + '' + event;
			var func = _self[funcStr];	
			//console.log('calling ' + 'draw'+mode+''+event);
			if(funcStr === 'drawShapeDown') {
				console.info('canvas offset:');
				console.info(_self.canvas);
				console.info(canvas_offset);
				console.info(_self.$elem.offset());
			}
		...
		/*******************************************************************************
		 * transfer control back to the fabric.Canvas to allow shape manipulation/free drawing
		 *******************************************************************************/
		setFabricMode: function(isFabric) {
			console.log('setting fabric = ' + isFabric);
			if(isFabric) {
				this.removeMyListeners();
				this.fabCanvas.addMyListeners();
			}else {
				this.fabCanvas.removeMyListeners();
				this.addMyListeners();
			}
		},
		...
		drawShapeDown: function(e, _self)
		{
			...
			var xo = e.pageX, yo = e.pageY;

			var shapeProps = { //needed for ellipse, rect, and line
				stroke: _self.hexToRGB(_self.settings.strokeStyle),
				strokeWidth: parseInt(_self.settings.lineWidth)
			}, shape;
			
			if(_self.settings.mode !== 'Line') {
				$.extend(shapeProps, { //needed for ellipse and rect
					left: xo,
					top: yo,
					originX: 'left',
					originY: 'top',
					width: 0,
					height: 0,
					fill: _self.hexToRGB(_self.settings.fillStyle),
				});
			}

			console.info('starting ' + _self.settings.mode + ' with props:');
			console.info(shapeProps);
			switch(_self.settings.mode) {
				case 'Ellipse':
					shapeProps.rx = 0;
					shapeProps.ry = 0;
					shape = new fabric.Ellipse(shapeProps);
					break;
				case 'Rectangle':
					shape = new fabric.Rect(shapeProps);
					break;
				case 'Line':
					shape = new fabric.Line([xo,yo,xo,yo], shapeProps);
					break;
			}
			if(shape) {
				console.info(shape);
				_self.fabCanvas.add(shape);
				_self.selectedShape = shape;
			}

			if(_self.settings.mode === 'Text') {
				_self.drawTextDown(e, _self);
				console.info(_self.textInput[0]);
				console.info(_self.textInput[0].parentNode);
			}
		...
		hexToRGB: function(str) {
			...
			for(var i = 0; i < 3; i++) {
				curVal = 0;
				for(var j = 0; j < 2; j++) {
					var hex = str.charCodeAt(2*i + j + 1);
					if(hex > 96) hex -= 87;
					else if(hex > 64) hex -= 55;
					else hex -= 48;
					curVal += (j == 0 ? 16 : 1) * hex;
				}
				ret += '' + curVal + (i < 2 ? ',' : '');
		...
		drawShapeMove: function(e, _self)
		{
			...
			if(_self.selectedShape) {
				if(_self.settings.mode !== 'Line') _self.selectedShape.set({left: left, top: top, width: width, height: height});
				switch(_self.settings.mode) {
					case 'Ellipse':
						_self.selectedShape.set({rx: width/2, ry: height/2});
						break;
					case 'Line':
						_self.selectedShape.set({x1: xo, y1: yo, x2: e.pageX, y2: e.pageY});
						break;
				}
				console.log('fabric rendering');
				_self.fabCanvas.renderAll();
			}
			if(_self.settings.mode == 'Text') {
				_self.textInput.css({left: left+1, top: top+1, width:width, height:height});
				//console.log('text: ' + _self.textInput.offset().left + ',' + _self.textInput.offset().top + 
				//	', ' + _self.textInput.width() + ',' + _self.textInput.height());
			}
		...
		drawShapeUp: function(e, _self)
		{
			...
			if(_self.selectedShape) {
				_self.selectedShape.setCoords(); //ensure bounding box is up to date
				_self.selectedShape = undefined;
			}
		},
		...
		drawTextUp: function(e, _self)
		{
			...
			if(_self.fabCanvas) {
				var lineHeight = parseInt(_self.textInput.css('line-height').replace('px',''))
					 / parseInt(_self.settings.fontSize);
				_self.fabCanvas.add(new fabric.Text(linesNew.join('\n'),
				{
					width: _self.textInput.width(),
					height: _self.textInput.height(),
					top: _self.textInput.offset().top - _self.$elem.offset().top,
					left: _self.textInput.offset().left - _self.$elem.offset().left,
					originX: 'left',
					originY: 'top',
					fontSize: _self.settings.fontSize,
					fontFamily: _self.settings.fontFamily,
					fontStyle: _self.settings.fontTypeItalic ? 'italic' : '',
					fontWeight: _self.settings.fontTypeBold ? 'bold' : '',
					fill: _self.hexToRGB(_self.settings.fillStyle),
					lineHeight: lineHeight
				}));
				console.log('line height = ' + _self.textInput.css('line-height'));
			}
			return;
		...
		getImage: function()
		{
			...
			console.info(this.canvasSave);
			console.info(this.ctxSave);
	...
	MainMenu.prototype = 
	{
		generate: function(canvas, textMenu)
		{
			...
			$.each($canvas.settings.menu, function(i, item)
			{
				switch(item)
				{
					...
					case 'clear': menuContent.append($('<div class="_wPaint_icon _wPaint_clear" title="' + $canvas.settings.menuTitles.clear + '"></div>').click(function(){ $canvas.fabCanvas.clear(); })); break;
					case 'select': menuContent.append($('<div class="_wPaint_icon _wPaint_select" title="' + $canvas.settings.menuTitles.select + '"></div>').click(function(){ _self.set_mode(_self, $canvas, 'Select'); })); break;
			...
			.css({position: 'absolute',
				left: $canvas.settings.menuOffsetX + $canvas.elemOffset.left,
				top: $canvas.settings.menuOffsetY + $canvas.elemOffset.top})
			.draggable({
				handle: menuHandle, 
		...
		set_mode: function(_self, $canvas, mode)
		{
			...
			if(mode == 'Text')
			{
				if($canvas.fabCanvas) $($canvas.fabCanvas.upperCanvasEl).hide();
				_self.textMenu.menu.show();
				_self.setWidth($canvas, _self.textMenu.menu);
			}
			else
			{
				if($canvas.fabCanvas) $($canvas.fabCanvas.upperCanvasEl).show();
			...
			if($canvas.fabCanvas) {
				if(mode == 'Select' || mode == 'Pencil') {
					$canvas.setFabricMode(true);
				}else {
					$canvas.setFabricMode(false);
				}

				if(mode == 'Pencil') {
					$canvas.fabCanvas.isDrawingMode = true;
					$canvas.fabCanvas.freeDrawingBrush.color = $canvas.hexToRGB($canvas.settings.strokeStyle);
					$canvas.fabCanvas.freeDrawingBrush.width = $canvas.settings.lineWidth;
				}else {
					$canvas.fabCanvas.isDrawingMode = false;
				}
				
				if(mode != 'Select') {
					$canvas.fabCanvas.discardActiveObject();
					$canvas.fabCanvas.discardActiveGroup();
				}
				
				$canvas.fabCanvas.renderAll();
			}
